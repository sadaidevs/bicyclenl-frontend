...
const reportResources: FinancialReportResource[] = [
  {
...
export default function FinancialReportsSection() {
  ...
  const totalPages = reportResources.length
  const activeReport = useMemo(() => {
    const safeIndex = Math.min(Math.max(currentPage, 1), totalPages) - 1
    return reportResources[safeIndex]
        ...
        <div className="rounded-3xl border border-slate-200 bg-linear-to-br from-slate-50 via-white to-blue-50 p-6 shadow-sm">
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px] lg:items-start">
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
              {activeReport ? (
                <Link
                  href={getDownloadHref(activeReport.fileName)}
                  target="_blank"
                ...
                >
                  ...
                  <p className="mt-3 text-sm leading-6 text-slate-600">{activeReport.description}</p>
            ...
            <aside className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5 lg:sticky lg:top-6 lg:self-start">
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">Report List</p>
              <div className="mt-4 max-h-50 space-y-2 overflow-y-auto pr-1 scroll-smooth snap-y snap-mandatory">
                {reportResources.map((report, index) => {
                  ...
                  return (
                    ...
                    >
                      <span className="min-w-0 pr-3 text-sm font-medium leading-5">{report.title}</span>
